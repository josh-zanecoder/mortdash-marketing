...
export async function GET(req: NextRequest) {
    try {
      ...
      if (!res.ok) {
        return NextResponse.json({ error: 'Failed to fetch prospects' }, { status: res.status });
      }
      ...
      return NextResponse.json({ error: 'Failed to fetch prospects', details: error.message }, { status: 500 });
    }
  }
