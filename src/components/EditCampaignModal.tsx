...
export default function EditCampaignModal({
  ...
  const { emailTemplates, emailTemplatesLoading, fetchEmailTemplates, updateCampaignDetails } = useCampaignStore();
  ...
  const fetchCampaignDetails = async () => {
    ...
    try {
      ...
      setSelectedListId(campaignData.marketingListId || campaignData.marketing_list_id || '');
      setSelectedTemplateId(
      ...
      if (campaignData.scheduledAt || campaignData.scheduled_at) {
        const scheduledDate = new Date(campaignData.scheduledAt || campaignData.scheduled_at);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!token || !campaignId) {
      toast.error('Token or campaign ID missing');
      return;
    }
    ...
    try {
      ...
      await updateCampaignDetails(token, campaignId, updateData);

      toast.success(
        scheduledAt 
          ? `Campaign "${campaignName}" updated and scheduled successfully!` 
          : `Campaign "${campaignName}" updated successfully!`
      );
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-6 px-6 pb-6 pt-4">
            ...
            <div className="flex justify-end gap-3 pt-4 border-t">
              ...
              <Button
                type="submit"
                disabled={loading}
                className="cursor-pointer px-6 py-2.5 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white rounded-lg shadow-md hover:shadow-lg transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed font-medium"
              >
                {loading ? (
                  <div className="flex items-center gap-2">
                    <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
                    {scheduledAt ? 'Updating...' : 'Updating...'}
                  </div>
                ) : (
                  scheduledAt ? 'Update & Schedule Campaign' : 'Update Campaign'
                )}
              </Button>
            </div>
          </form>
