...
export default function DeleteMarketingListDialog({ 
  open, 
  onClose, 
  onConfirm, 
  list 
  ...
  const listName = list?.listName || list?.list_name || 'this marketing list';
    ...
    <Dialog open={open} onOpenChange={v => { if (!v) onClose(); }}>
      <DialogContent className="max-w-md w-full p-0 rounded-2xl">
        <DialogHeader className="flex flex-col items-center justify-center pt-8 pb-2">
          ...
          <DialogTitle className="text-2xl font-bold text-gray-900">Delete Marketing List</DialogTitle>
        </DialogHeader>
        <div className="px-8 pb-8 text-center">
          <p className="mb-6 text-base text-gray-700">
            Are you sure you want to delete <span className="font-semibold text-gray-900">"{listName}"</span>?
